...
from resnest.torch import resnest50 # Importing ResNeSt model
...
# Initialize ResNeSt model
resnest = resnest50(pretrained=False)
weight_path = '../model/resnest50-528c19ca.pth'
resnest.load_state_dict(torch.load(weight_path))
resnest.to(device)
...
num_features = resnest.fc.in_features
resnest.fc = nn.Linear(num_features, 2).to(device)

# Define optimizer and loss function
optimizer = torch.optim.Adam(resnest.fc.parameters(), lr=0.0001)  # Optimizing only the classifier layer
...
print("Evaluation after training with ResNeSt:")
...
gflops = calculate_gflops(resnest, input_tensor)
