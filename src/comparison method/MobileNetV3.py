...
mobilenetv3 = models.mobilenet_v3_small(pretrained=True).to(device)

# Modify the last fully connected layer for the binary classification task
num_features = mobilenetv3.classifier[-1].in_features
mobilenetv3.classifier[-1] = nn.Linear(num_features, 2).to(device)

# Define optimizer and loss function
optimizer = torch.optim.Adam(mobilenetv3.parameters(), lr=0.0001)
...
print("Evaluation after training with MobileNetV3:")
...
gflops = calculate_gflops(mobilenetv3, input_tensor)
